...
export function handleBreakupApproved(event: BreakupApprovedEvent): void {
    let entity = new BreakupApproved(event.transaction.hash.toHex() + "-" + event.logIndex.toString());
...
export function handleBreakupProposal(event: BreakupProposalEvent): void {
    let entity = new BreakupProposal(event.transaction.hash.toHex() + "-" + event.logIndex.toString());
    entity.relationshipId = event.params.relationshipId;
    entity.initiator = event.params.initiator;
    entity.save();
}

export function handleRelationshipApproved(event: RelationshipApprovedEvent): void {
    let entity = new RelationshipApproved(event.transaction.hash.toHex() + "-" + event.logIndex.toString());
    entity.relationshipId = event.params.relationshipId;
...
export function handleRelationshipEvent(event: RelationshipEventEvent): void {
    let entity = new RelationshipEvent(event.transaction.hash.toHex() + "-" + event.logIndex.toString());
...
export function handleRelationshipProposed(event: RelationshipProposedEvent): void {
    let entity = new RelationshipProposed(event.transaction.hash.toHex() + "-" + event.logIndex.toString());
    entity.relationshipId = event.params.relationshipId;
    entity.firstHalf = event.params.firstHalf;
    entity.secondHalf = event.params.secondHalf;
    entity.save();
